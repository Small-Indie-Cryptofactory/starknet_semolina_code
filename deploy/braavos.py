...
from deploy.braavos_signature import deploy_account_braavos


class Braavos(Base):
    async def wallet_deploy(self, depth=10):
        if depth == 0:
            raise ValueError('Failed to deploy account (braavos).')
        ...
        try:
            ...
            if result:
                ...
                msg = (f'Braavos wallet: {wallet_address}. Deploy successfully. Tx hash: '
