...
class BraavosUpgrade(Base):
    implementation = Implementations.Braavos

    async def _get_impl_version(self, contract, depth=10):
        if depth == 0:
            return (f'Failed | Not possible receive impl_version for braavos wallet '
                    f'{self.starknet_client.hex_address}. Starknet nodes problem.')
        try:
            impl_version = await contract.functions['get_impl_version'].call()
            version = bytes.fromhex(hex(impl_version.as_tuple()[0])[2:]).decode('utf-8')
            return version
        except (ClientError, ClientConnectorError):
            return await self._get_impl_version(contract, depth-1)

    async def braavos_wallet_upgrade(self, depth=10):
        """ Upgrade braavos wallet """

        if depth == 0:
            return (f'Failed upgrade -> "The server encountered a temporary error and could not complete your request"'
                    f' Wallet {self.starknet_client.hex_address}')

        failed_text = f'Failed braavos upgrade, wallet: {self.starknet_client.hex_address}'
        try:
            wallet_contract = await Contract.from_address(
            ...
            if impl_version == BraavosUpgrade.implementation.actual_version:
                return f'Braavos wallet {self.starknet_client.hex_address} already have actual version {impl_version}'
            else:
                upgrade_call = wallet_contract.functions['upgrade'].prepare(
                    new_implementation=BraavosUpgrade.implementation.contract_address
                )
                ...
                return (f'Braavos wallet {self.starknet_client.hex_address} successfully upgrade | '
            ...
            return self.braavos_wallet_upgrade(depth-1)
